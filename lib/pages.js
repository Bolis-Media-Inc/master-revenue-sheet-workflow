...
/**
 * Levenshtein edit distance (in-place DP, O(m*n) time, O(n) space).
 * Used by resolveHandle for typo-tolerant page lookup.
 */
...
/**
 * Fuzzy-resolve a brief-time handle against the registry. Useful when
 * Danielson typos a handle (e.g. @dankquilius / one L instead of the
 * actual @dankquillius / two L's) — bot would otherwise treat it as
 * an unknown page and skip the per-page sheet write + forward.
 *
 * Strategy:
 *   1. Exact match in registry → return canonical handle as-is
 *   2. No exact match → find registry handles within Levenshtein
 *      distance ≤ maxDist (default 1)
 *   3. If EXACTLY ONE registry handle matches at the minimum found
 *      distance, return that canonical handle (logged so we can audit)
 *   4. If two+ handles tie at the same min distance → ambiguous, return
 *      null (caller falls back to skip)
 *
 * Distance 1 catches single-character typos (missing/added/swapped/
 * substituted char). Going to 2 would catch more but starts to risk
 * false positives (e.g. @hoodreels vs @hoodgrels at distance 2).
 *
 * Returns the canonical handle (lowercased) or null. Always safe to
 * use — exact matches return immediately, no fuzzy work done.
 *
 * @param {string} handle  Raw handle from a brief (with or without @)
 * @param {number} [maxDist=1]
 * @returns {string|null}  Canonical registry handle, or null if unresolved
 */
function resolveHandle(handle, maxDist = 1) {
  if (!handle) return null;
  const norm = _norm(handle);
  // 1. Exact match
  if (_byHandle.has(norm)) return norm;

  // 1b. Separator-insensitive exact match. digi (and some upload tools)
  // sanitize "_" and "." to "-" in cover filenames, so a cover named
  // "@i-have-no-memes96-v2.jpg" must resolve to page "@i_have_no_memes96_v2"
  // and "@thefuck-tv.jpg" to "@thefuck.tv". Levenshtein ≤1 only catches the
  // single-separator cases (thefuck-tv), never the multi-separator ones, and
  // would risk false positives at higher distance. Treating -, _, . as one
  // equivalence class is deterministic: collapse them and require a UNIQUE
  // registry handle with the same collapsed form.
  const bare = norm.replace(/[-_.]/g, "");
  if (bare) {
    const sepMatches = [];
    for (const candidate of _byHandle.keys()) {
      if (candidate.replace(/[-_.]/g, "") === bare) sepMatches.push(candidate);
    }
    if (sepMatches.length === 1) {
      if (sepMatches[0] !== norm) {
        console.log(`[pagesRegistry] 🔗 Separator-resolved @${norm} → @${sepMatches[0]}`);
      }
      return sepMatches[0];
    }
    if (sepMatches.length > 1) {
      console.warn(`[pagesRegistry] ⚠️ Ambiguous separator match for @${norm} — ${sepMatches.length} candidates, falling back to fuzzy`);
    }
  }

  // 2. Fuzzy scan
  let best = { handle: null, dist: Infinity, ties: 0 };
  for (const candidate of _byHandle.keys()) {
    // Only consider candidates roughly the same length — saves time and
    // avoids matching e.g. "moist" against "moistmemorablemedia"
    if (Math.abs(candidate.length - norm.length) > maxDist) continue;
    const d = _editDist(norm, candidate);
    if (d > maxDist) continue;
    if (d < best.dist) {
      best = { handle: candidate, dist: d, ties: 1 };
    } else if (d === best.dist) {
      best.ties++;
    }
  }
  if (best.handle && best.ties === 1) {
    console.log(`[pagesRegistry] 🔎 Fuzzy-resolved @${norm} → @${best.handle} (distance ${best.dist})`);
    return best.handle;
  }
  if (best.ties > 1) {
    console.warn(`[pagesRegistry] ⚠️ Ambiguous fuzzy match for @${norm} — ${best.ties} candidates at distance ${best.dist}, skipping`);
  }
  return null;
}
