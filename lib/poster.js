...
async function sendMedia(telegram, chatId, url, opts = {}) {
  if (!url) return;
  try {
    if (isVideoUrl(url)) {
...
async function sendCollabToInternal(telegram, payload, sessionId) {
  for (const video of (payload.videos || [])) {
    if (video.mediaUrl) await sendMedia(telegram, TARGET_CHAT, video.mediaUrl);
...
async function sendPerPageToInternal(telegram, payload, sessionId) {
  for (const page of (payload.pages || [])) {
    if (page.creativeUrl) {
      await sendMedia(telegram, TARGET_CHAT, page.creativeUrl);
      await telegram.sendMessage(TARGET_CHAT, `@${normalize(page.handle)} ^`);
    }
  }
  // Optional shared media (e.g. story media)
  if (Array.isArray(payload.sharedMedia)) {
    for (const m of payload.sharedMedia) {
      await sendMedia(telegram, TARGET_CHAT, m.url);
...
async function sendStandardToInternal(telegram, payload, sessionId) {
  // sharedSlides is the array of all slide URLs that go to every page
  const slides = payload.sharedSlides || (payload.sharedCreativeUrl ? [payload.sharedCreativeUrl] : []);
  for (const url of slides) await sendMedia(telegram, TARGET_CHAT, url);
  if (payload.adInfo.caption) await telegram.sendMessage(TARGET_CHAT, payload.adInfo.caption);
...
async function sendHybridToInternal(telegram, payload, sessionId) {
  ...
  // Then shared content slides
  const sharedSlides = payload.sharedSlides || [];
  for (const url of sharedSlides) await sendMedia(telegram, TARGET_CHAT, url);
...
async function sendToPageChannel(telegram, payload, page, sessionId, briefText) {
  ...
  switch (payload.format) {
    case "collab": {
      ...
      if (found) {
        if (found.video.mediaUrl) await sendMedia(telegram, dest, found.video.mediaUrl);
    ...
    case "per-page": {
      ...
      if (pagePayload?.creativeUrl) await sendMedia(telegram, dest, pagePayload.creativeUrl);
      if (Array.isArray(payload.sharedMedia)) {
        for (const m of payload.sharedMedia) await sendMedia(telegram, dest, m.url);
      }
      break;
    }
    ...
    case "standard": {
      const slides = payload.sharedSlides || (payload.sharedCreativeUrl ? [payload.sharedCreativeUrl] : []);
      for (const url of slides) await sendMedia(telegram, dest, url);
      break;
    }
    ...
    case "hybrid": {
      ...
      for (const url of (payload.sharedSlides || [])) await sendMedia(telegram, dest, url);
      break;
    }
  }
