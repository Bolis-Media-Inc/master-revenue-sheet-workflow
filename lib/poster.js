...
async function postReviewCard(bot, session, payload) {
  ...
  const submitterTag = submitter.email
    ? `${submitter.email}`
 ...
 */
async function approveSession(bot, sessionId, approverTelegramId) {
  const { data: session } = await sessions._supabase
    .from("ad_sessions")
    .select("*")
    .eq("id", sessionId)
 ...
 */
async function rejectSession(bot, sessionId, approverTelegramId, reason = "") {
  const { data: session } = await sessions._supabase
    .from("ad_sessions")
    .select("*")
    .eq("id", sessionId)
  ...
  if (session.review_msg) {
    try {
      ...
      await bot.telegram.editMessageText(
        ...
        session.review_msg.messageId,
        undefined,
        `❌ *Rejected* — not sent.\n\n_Rejected by ${approverTelegramId}_${note}`,
        { parse_mode: "Markdown" },
      );
