...
/**
 * Return all not-yet-finished pending briefs for a chat (status pending or
 * processing), excluding one message_id. Used by the supersede guard: when a
 * new copy of a brief is deferred, we look up the older copies for the same
 * chat so we can cancel the ones for the same campaign (operator deleted +
 * re-sent to fix a typo / add a creative).
 *
 * Returns [] on any error or when disabled — supersede is best-effort.
 */
async function listOpenForChat(chatId, excludeMessageId) {
  if (!supabase) return [];
  try {
    const { data, error } = await supabase
      .from("pending_briefs")
      .select("chat_id, message_id, scheduled_for")
      .eq("chat_id", Number(chatId))
      .in("status", ["pending", "processing"])
      .neq("message_id", Number(excludeMessageId));
    if (error) {
      console.error(`[pendingBriefs] listOpenForChat error: ${error.message}`);
      return [];
    }
    return data || [];
  } catch (err) {
    console.error(`[pendingBriefs] listOpenForChat threw: ${err.message}`);
    return [];
  }
}

/**
 * Cancel a pending brief because a newer copy of the same campaign was
 * posted (delete + re-send workflow). Marked 'superseded' so it never
 * forwards; distinct from 'processed' for audit clarity.
 */
async function markSuperseded(chatId, messageId, bySupersedingMessageId) {
  if (!supabase) return;
  await supabase
    .from("pending_briefs")
    .update({
      status:     "superseded",
      last_error: `superseded by msg ${bySupersedingMessageId}`,
    })
    .eq("chat_id",    Number(chatId))
    .eq("message_id", Number(messageId));
}
