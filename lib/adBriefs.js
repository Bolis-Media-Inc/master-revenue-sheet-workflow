...
async function markPageForwarded(pageRowId, outcome = {}) {
  ...
  // Only WRITE a field when a real value is supplied — NEVER null one out.
  // Callers (live forward, /replay, takedown) routinely pass stale/empty
  // values here; an unconditional write CLOBBERS data set moments earlier.
  // That's how forwarded_message_ids got wiped on 178 forwarded rows (so
  // /remove couldn't find the posts to delete → "Chat no msgid") and how 741
  // rows lost page_sheet_row. Each guard makes this update ADD-only.
  if (Array.isArray(outcome.messageIds) && outcome.messageIds.length) patch.forwarded_message_ids = outcome.messageIds;
  ...
  if (outcome.ledgerSheetRow != null) patch.ledger_sheet_row = outcome.ledgerSheetRow;
 ...
 */
async function updatePageSheetRows(pageRowId, { masterSheetRow, pageSheetRow, ledgerSheetRow }) {
  if (!supabase || !pageRowId) return;
  const patch = {};
  if (masterSheetRow != null) patch.master_sheet_row = masterSheetRow;
  if (pageSheetRow   != null) patch.page_sheet_row   = pageSheetRow;
  if (ledgerSheetRow != null) patch.ledger_sheet_row = ledgerSheetRow;
...
async function getBriefPages(briefId, handleFilter = null) {
  ...
  let q = supabase
    .from("ad_brief_pages")
    .select("id, page_handle, bulk_num, page_price, page_media, page_caption, forwarded_at, forwarded_message_ids, master_sheet_row, page_sheet_row, ledger_sheet_row")
    .eq("brief_id", briefId)
 ...
 */
/**
 * Pages that were FORWARDED SUCCESSFULLY but have no tracked per-page sheet
 * row (page_sheet_row IS NULL). These are the candidates for "sent but never
 * sheeted" — the /audit reconciliation then verifies each against the actual
 * sheet, since a NULL row often just means the DB lost track (clobber bug),
 * not that the row is genuinely absent.
 *
 * forward_error rows are excluded (those didn't actually send). Removed ads
 * are naturally absent — a takedown DELETEs the ad_brief_pages row.
 */
async function findForwardedMissingPageRows(opts = {}) {
  if (!supabase) return [];
  const limit = opts.limit ?? 2000;
  const { data, error } = await supabase
    .from("ad_brief_pages")
    .select(`
      id, brief_id, page_handle, bulk_num, page_price, forwarded_at, posted_at,
      brief:ad_briefs!brief_id( id, client, category, total_price, date_posted, time_mst )
    `)
    .not("forwarded_at", "is", null)
    .is("forward_error", null)
    .is("page_sheet_row", null)
    .order("forwarded_at", { ascending: false })
    .limit(limit);
  if (error) {
    console.error("[adBriefs] findForwardedMissingPageRows error:", error.message);
    return [];
  }
  return (data || []).map((r) => ({
    ...r,
    brief: Array.isArray(r.brief) ? r.brief[0] : r.brief,
  }));
}
...
/**
 * Source-of-truth lookup for the dupe verifier: every brief that targeted a
 * given page, optionally filtered to one client. Returns the brief's date +
 * Telegram message id so the caller can count DISTINCT real posts (= distinct
 * messages in Internal Network Ads) for a (page, client, date) and compare it
 * against how many rows exist in the sheet. More sheet rows than distinct
 * messages = true duplicates; equal = legit repeats.
 *
 * @param {string} pageHandle
 * @param {string} [client]  exact (case-insensitive) client-name filter
 * @returns {Promise<Array<{client:string, date_posted:string, telegram_message_id:number}>>}
 */
async function getBriefsForPageClient(pageHandle, client) {
  if (!supabase || !pageHandle) return [];
  const { data, error } = await supabase
    .from("ad_brief_pages")
    .select("brief:ad_briefs!brief_id(client, date_posted, telegram_message_id)")
    .eq("page_handle", String(pageHandle).toLowerCase());
  if (error) {
    console.error("[adBriefs] getBriefsForPageClient error:", error.message);
    return [];
  }
  const want = (client || "").trim().toLowerCase();
  return (data || [])
    .map((r) => (Array.isArray(r.brief) ? r.brief[0] : r.brief))
    .filter((b) => b && (!want || (b.client || "").trim().toLowerCase() === want));
}
...
module.exports = {
  ...
  findIncompletePages,
  findForwardedMissingPageRows,
  getBriefPages,
  getBriefsForPageClient,
  listRecentBriefs,
  markPagesPosted,
