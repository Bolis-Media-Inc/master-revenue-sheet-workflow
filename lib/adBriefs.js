 ...
 * @param {string[]} [brief.sharedMediaFileIds]
...
async function insertBrief(brief) {
  ...
  const row = {
    ...
    shared_media_file_ids: brief.sharedMediaFileIds ?? [],
...
async function insertBriefPages(briefId, pages) {
  ...
  const rows = pages.map((p) => ({
    ...
    page_media_file_ids: p.pageMediaFileIds ?? [],
...
async function findBriefsByClient(clientQuery, limit = 5) {
  ...
  const { data, error } = await supabase
    .from("ad_briefs")
    .select("id, client, raw_text, received_at, telegram_chat_id, telegram_message_id, shared_media_file_ids, shared_caption")
...
async function getBriefPages(briefId, handleFilter = null) {
  ...
  let q = supabase
    .from("ad_brief_pages")
    .select("id, page_handle, bulk_num, page_price, page_media_file_ids, page_caption, forwarded_at, master_sheet_row, page_sheet_row")
    .eq("brief_id", briefId)
