...
// During shadow we LOG every comparison to the DB but only PING Telegram for the
// money-losing cases (caption dropped / caption suspect) to respect the
// minimize-noise rule. Set BRIEF_AI_FLAG_ALL=true to ping on every disagreement.
...
async function shadowCompare(telegram, alertChatId, info) {
  ...
  try {
    ...
    // Only ping Telegram for caption-money cases unless FLAG_ALL is set.
    const moneyCase = diffs.some((d) => d.startsWith("🟥") || d.startsWith("🟧"));
    if (!telegram || !alertChatId) return;
    if (!FLAG_ALL && !moneyCase) return;
