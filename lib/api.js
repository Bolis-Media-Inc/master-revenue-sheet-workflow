...
function createServer({ bot, handleIntake }) {
  const server = http.createServer(async (req, res) => {
    ...
    // Chat-discovery endpoints (/api/chats/search, /api/admin/invite-bot)
    // used to live here but were removed — they required a gramJS user
    // session, which kept conflicting with other processes using the same
    // Telegram identity. User-account features belong in Digi, not Greg.
    // The page-registry's "chat ID capture" now relies on the bot's own
    // my_chat_member listener (index.js) which DMs the admin with each
    // chat ID the moment @bm_tracking_bot is added to a chat.
...
function startServer({ bot, handleIntake }) {
  ...
  server.listen(PORT, async () => {
    ...
    // Reconcile bulk templates from the durable Supabase mirror BEFORE
    // serving reads. The container filesystem resets to the git-committed
    // bulks.json on every redeploy/restart; without this, web-created
    // templates (which live only in Supabase after a restart) stay
    // invisible because every read consults the disk file.
    try {
      const bulkTemplates = require("./bulkTemplates");
      await bulkTemplates.hydrateFromSupabase();
    } catch (err) {
      console.error("❌ bulk template hydration failed:", err.message);
    }
  });
  return server;
}
