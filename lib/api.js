...
function createServer({ bot, handleIntake }) {
  const server = http.createServer(async (req, res) => {
    ...
    // ── POST /api/admin/invite-bot ──────────────────────────────────────
    // One-shot bulk-invite of @bm_tracking_bot (or BOT_USERNAME override)
    // into every chat in the pages registry. Uses the ops Telegram user
    // session if configured, else falls back to sales. Skips USER_ALREADY_
    // PARTICIPANT silently. Returns a JSON summary so the caller (curl
    // from any terminal) sees exactly what happened.
    //
    // Body (optional):
    //   { botUsername?: "bm_tracking_bot", dryRun?: false }
    if (method === "POST" && url === "/api/admin/invite-bot") {
      let body = {};
      try { body = (await readBody(req)) || {}; } catch (e) { /* allow empty body */ }
      const botUsername = String(body.botUsername || "bm_tracking_bot").replace(/^@/, "");
      const dryRun = !!body.dryRun;

      try {
        // Lazy-require so the API works even if GramJS env is missing in
        // some deploys (we just return a friendly error in that case).
        const userClient = require("../userClient");
        const { Api } = require("telegram");

        // Prefer ops (admin in every chat by creation), fall back to sales
        // on any failure (commonly: ops SESSION_REVOKED). We probe by
        // calling getEntity on the bot username — if that throws, ditch
        // the ops client and retry with sales for the whole operation.
        let client = (await userClient.getOpsClient()) || (await userClient.getClient());
        if (!client) {
          return send(res, 503, { error: "Neither ops nor sales Telegram session available" });
        }
        let botEntity;
        let activeAccount = "ops";
        try {
          botEntity = await client.getEntity(botUsername);
        } catch (e) {
          console.warn(`[api] bulk-invite: ops client failed (${e.message}) — falling back to sales`);
          // Invalidate cached ops client so subsequent calls retry the env
          if (client === (await userClient.getOpsClient())) {
            try { await client.disconnect(); } catch (_) {}
          }
          client = await userClient.getClient();
          botEntity = await client.getEntity(botUsername);
          activeAccount = "sales";
        }

        const { data: pages, error: dbErr } = await sessions._supabase
          .from("pages")
          .select("handle, chat_id")
          .not("chat_id", "is", null)
          .order("handle");
        if (dbErr) return send(res, 500, { error: `pages read: ${dbErr.message}` });
        ...
        for (const page of pages) {
          ...
          let entity;
          try {
            entity = await client.getEntity(page.chat_id);
          } catch (e) {
            counts.failed++;
            log.push({ handle: page.handle, chatId: page.chat_id, status: "fetch_failed", reason: e.message });
            continue;
          }

          if (dryRun) {
            counts.skipped++;
            log.push({ handle: page.handle, chatId: page.chat_id, status: "would_invite", title: entity.title || entity.username });
            continue;
          }
          ...
          try {
            if (entity.className === "Channel") {
              await client.invoke(new Api.channels.InviteToChannel({
                channel: entity, users: [botEntity],
              }));
            } else if (entity.className === "Chat") {
              await client.invoke(new Api.messages.AddChatUser({
                chatId: entity.id, userId: botEntity, fwdLimit: 0,
              }));
            } else {
              counts.failed++;
              log.push({ handle: page.handle, chatId: page.chat_id, status: "unsupported_entity", reason: entity.className });
              continue;
            }
            counts.invited++;
            log.push({ handle: page.handle, chatId: page.chat_id, status: "invited" });
          } catch (e) {
            const msg = e.message || String(e);
            if (/USER_ALREADY_PARTICIPANT|ALREADY_PARTICIPANT/i.test(msg)) {
              counts.alreadyIn++;
              log.push({ handle: page.handle, chatId: page.chat_id, status: "already_in" });
            } else if (/CHAT_ADMIN_REQUIRED|CHAT_WRITE_FORBIDDEN/i.test(msg)) {
              counts.failed++;
              log.push({ handle: page.handle, chatId: page.chat_id, status: "needs_manual_add", reason: "ops/sales not admin in this chat" });
            } else if (/USER_PRIVACY_RESTRICTED|USER_BANNED_IN_CHANNEL/i.test(msg)) {
              counts.failed++;
              log.push({ handle: page.handle, chatId: page.chat_id, status: "needs_manual_add", reason: "bot privacy/banned" });
            } else {
              counts.failed++;
              log.push({ handle: page.handle, chatId: page.chat_id, status: "error", reason: msg });
            }
          }

          // Polite delay (skip on dry run)
        ...
        return send(res, 500, { error: `bulk-invite failed: ${e.message}` });
      }
    }
